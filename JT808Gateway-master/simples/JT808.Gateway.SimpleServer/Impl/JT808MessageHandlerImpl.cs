        ...
        public override byte[] Msg0x0200(JT808HeaderPackage request)
{
    ...
    {
        logger.LogDebug($"Received Msg0x0200 from {request.Header.TerminalPhoneNo}");
    }

    // Send JT1078 video stream start command (0x9101)
    var terminalPhoneNo = request.Header.TerminalPhoneNo;

    Task.Run(async () =>
    {
        try
        {
            var triggerService = JT808GatewaySimpleServerHost.ServiceProvider.GetRequiredService<JT1078VideoTriggerService>();
            await triggerService.SendVideoStreamCommand(terminalPhoneNo);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, $"Failed to send 0x9101 to {terminalPhoneNo}");
        }
    });
